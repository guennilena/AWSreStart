...
# ./challenge.sh file_folder file_name num_files

file_folder="$1"
file_name="$2"
num_files="$3"

# Überprüfen ob die Syntaxregel eingehalten wurde
if [[ -z "$file_folder" || -z "$file_name" || -z "$num_files" ]]; then
  echo "Nutzung: $0 Ordnername Dateiname AnzahlDateien"
  exit 1
fi
...
if [ ! -d "$file_folder" ]; then
  ...
  echo "Drücken sie J zum Anlegen. <anykey> zum Beenden"
  read -n 1 input
